...
public class PocWebClient {
	...
	public static void main(String[] args) {
		...
		if (args.length < 3) {
			System.out.println("Invalid execution! Please run using the following arguments:\n"
					+ "proxy address (mandatory)" + "proxy port (mandatory)" + "host address (mandatory)"
		...
		// Send request
		processRequest(URL, PROXY_ADDRESS, PROXY_PORT);
	}

	private static void processRequest(String URL, String PROXY_ADDRESS, String PROXY_PORT) {
		// TODO Auto-generated method stub
		InputStream input;
		OutputStream output;
		try {
			...
			System.out.println("Connected to proxy server at " + PROXY_ADDRESS + ":" + PROXY_PORT);
			...
			// Checking if resource type is html or txt
			if ((URL.substring(URL.lastIndexOf(".")) == "html" || URL.substring(URL.lastIndexOf(".")) == "txt")
					&& FILE_PATH == null) {
				while (true) {
					int num = 0;
					...
					if (Files.exists(Paths.get(FILE_PATH))) {
						String fileName = FILE_PATH + UUID.randomUUID() + URL.substring(URL.lastIndexOf("/") + 1);
					...
					String fileName = "./" + UUID.randomUUID() + URL.substring(URL.lastIndexOf("/") + 1);
					FileOutputStream fileOutput = new FileOutputStream(fileName);
					byte[] data = new byte[1024];
					int count;
					while ((count = input.read(data)) != -1) {
	...
	private static void validateInput(String URL, String PROXY_ADDRESS, String PROXY_PORT) {
		...
		try {
			if (URL.length() == 0 || !((new URL(URL)) instanceof URL)) {
				System.out.println("Please provide a valid host address, e.g.  http://127.0.0.1:8000/testfile.html.");
				System.exit(1);
			}
